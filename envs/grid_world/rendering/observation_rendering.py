...
class ObservationRenderer(object):
    def __init__(self, cfg):
        ...
        assert model_utils.is_odd(
            self.obs_resolution[0]) and model_utils.is_odd(
        ...
        self.drawing = np.zeros((self.n_channels, self.global_resolution[0],
                                 self.global_resolution[1]), dtype=np.uint8)
    ...
    def get_frame_at_point(self, center: Iterable[float]):
        ...
        frame = np.zeros(
            (self.n_channels, self.obs_window[0], self.obs_window[1]))
        ...
        row_end = int(
            min(self.global_resolution[0], frame_center[0] + center[0] + 1))
        col_end = int(
            min(self.global_resolution[1], frame_center[1] + center[1] + 1))
        ...
        target_row_end = int(
            min(self.obs_window[0], target_row_start + drawing_slice.shape[-2]))
        target_col_end = int(
            min(self.obs_window[1], target_col_start + drawing_slice.shape[-1]))
    ...
    def convert_location_to_pixels(self, location: Iterable[float],
                                   origin_bounds: Iterable[Iterable[float]]):
        '''
        ...
        '''
        y_scale = (location[0] - origin_bounds[0][0]) / (
                    origin_bounds[0][1] - origin_bounds[0][0])
        x_scale = (location[1] - origin_bounds[1][0]) / (
                    origin_bounds[1][1] - origin_bounds[1][0])
...
if __name__ == '__main__':
    renderer = ObservationRenderer(resolution=100)
