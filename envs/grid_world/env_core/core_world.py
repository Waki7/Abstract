...
class CoreWorld(object):
    ...
    def get_object_coordinates(self):
        ...
        agent_coordinates = [agent.location for agent in
                             self.agent_map.values()]
        landmark_coordinates = [landmark.location for landmark in
                                self.landmark_map.values()]
        return agent_coordinates + landmark_coordinates

    def spawn_agents(self, agents: List[env_agents.EnvAgent],
                     locations: List[Iterable[int]]):
        [self.spawn_agent(agent, location) for agent, location in
         zip(agents, locations)]

    def spawn_agent(self, agent: env_agents.EnvAgent, location: Iterable[int]):
        self.agent_map[agent.id] = agent
        agent.place(location)

    def spawn_landmarks(self, landmarks: List[env_landmarks.Landmark],
                        locations: List[Iterable[int]]):
        [self.spawn_landmark(landmark, location) for landmark, location in
         zip(landmarks, locations)]

    def spawn_landmark(self, landmark: env_landmarks.Landmark,
    ...
    def is_object_of_bounds(self, object: env_objects.GridObject) -> bool:
    ...
    def move_agent(self, agent: env_agents.EnvAgent, action: np.ndarray,
                   illegal_func=None):
    ...
    def get_distance(self, obj1: env_objects.GridObject,
                     obj2: env_objects.GridObject):
    ...
    def render_world(self):
        ...
        for agent in self.agent_map.values():
            pixel_location = self.renderer.convert_location_to_pixels(
    ...
    def get_random_point(self):
        ...
        rand_y = np.random.randint(low=(self.bounds[0][0] * granularity) + 1.,
                                   high=self.bounds[0][1] * granularity)
        rand_x = np.random.randint(low=(self.bounds[1][0] * granularity) + 1.,
                                   high=self.bounds[1][1] * granularity)
    ...
    def get_agent_pov(self, agent: env_agents.EnvAgent):
        assert agent.id in self.agent_map, 'agent has not been spawned in world'
        pixel_location = self.renderer.convert_location_to_pixels(
            location=agent.location, origin_bounds=self.bounds)
