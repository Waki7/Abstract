...
class ChannelNetwork(BaseNetwork):
    def __init__(self, in_shape, out_shape, cfg,
        ...
        for channel_idx in range(0, self.n_out_channels):
            new_layer = nn.Linear(
                in_features=self.wl2.out_features, out_features=self.out_shapes[channel_idx], bias=self.bias)
            self.out_fcs.append(new_layer)

        self.hidden_input = torch.zeros((1, self.hidden_in_size)) # 1 for batch size

        self.create_optimizer()

    def forward(self, env_input, hidden_input = None):
        if hidden_input is None:
            hidden_input = self.hidden_in_size
